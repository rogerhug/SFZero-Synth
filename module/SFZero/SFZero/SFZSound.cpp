...
void SFZero::SFZSound::addNotError(const juce::String& message)
{
	notErrors.add(message);
}
	...
	double* progressVar, Thread* thread)
{
	...
	for (HashMap<String,SFZSample*>::Iterator i(samples); i.next();) {
		...
		addNotError( sample->getShortName());
		if (!ok)
...
String SFZero::SFZSound::getSampelsT()

{
	String result;
	int numErrors = notErrors.size();
	for (int i = 0; i < numErrors; ++i)
		result += notErrors[i] + "\n";
