...
SFZeroEditor::SFZeroEditor(SFZeroAudioProcessor& ownerFilter)
	...
	parameters.attachControls(
		...
		, audioProcessor.gParas.OverDModeCombo
	...
	audioProcessor.gParas.PitchSlider.onValueChange = [this] {audioProcessor.synthRate(audioProcessor.gParas.PitchSlider.getValue()); };
	...
	audioProcessor.gParas.OverDModeCombo.onChange = [this] {audioProcessor.updateOverDrive(); };
...
void SFZeroEditor::resized()
{
	...
	audioProcessor.gParas.setBounds(220, 30, 580, 250);
  // SfZProgrammLabel.setBounds(200, 20, 400, 20);
    ...
    textEntryBox.setBounds(20, 80, 200, 210);
...
{ 
	...
	sprintf(lbt, "midiIn->::channel %i::note %i::bender %i::rateHz %i",
		audioProcessor.readInt[1]
		, audioProcessor.readInt[0]
		, audioProcessor.readInt[2]
		, audioProcessor.readInt[3]
