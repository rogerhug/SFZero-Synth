...
void SFZeroAudioProcessor::applyGain(AudioBuffer<FloatType>& buffer)
{
	...
	smlevel.a1(0.95);
	float egGainAmp = 1;
	const float egGainAmpF = 0.001+((double)ampEG.getValue() * parameters.mainVolumeModValue)*2;
    const float gainLevel = 0.01+smlevel1.Process( egGainAmpF*parameters.mainVolumeValue);
	float freq = 20002 * parameters.egSmSensValue;

	if (parameters.egmode.getIndex() == 0) {
		egGainAmp = 1; freq = 22000;  
	}
